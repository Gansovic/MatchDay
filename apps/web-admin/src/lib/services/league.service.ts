...
export interface PublicLeagueFilters extends LeagueFilters {
  featured?: boolean;
...
export class LeagueService {
  ...
  /**
   * LEAGUE PUBLISHING METHODS
   * Extension to support Copa Facil-style league publishing
   */

  /**
   * Publish a league for public team registration
   */
  async publishLeague(data: PublishLeagueData): Promise<ServiceResponse<League>> {
    try {
      const updateData: any = {
        is_public: data.isPublic,
        auto_approve_teams: data.autoApproveTeams || false,
        registration_deadline: data.registrationDeadline,
        max_teams: data.maxTeams,
        featured: data.featured || false
      };

      // Set published_at timestamp when making public for first time
      if (data.isPublic) {
      ...
      // Clear cache for affected league
      this.clearCache(data.leagueId);
      ...
      return {
        ...
        error: null,
        success: true,
        message: data.isPublic ? 'League published successfully' : 'League unpublished'
      };
    } catch (error) {
      return {
        data: null,
  ...
  /**
   * Get featured public leagues (Copa Facil homepage style)
   */
  async getFeaturedLeagues(
    options: {
      limit?: number;
      sportType?: SportType;
    } = {}
    ...
    try {
      const cacheKey = this.getCacheKey('getFeaturedLeagues', options);
      const cached = this.getFromCache<LeagueDiscovery[]>(cacheKey);
      ...
      // Only include leagues with open registration
      const now = new Date().toISOString();
      ...
      // Process leagues into discovery format
      const featuredLeagues: LeagueDiscovery[] = await Promise.all(
        (leagues || []).map(async (league) => {
          const teams = league.teams || [];
          const playerCount = await this.getLeaguePlayerCount(league.id);
          const availableSpots = await this.getLeagueAvailableSpots(league.id);
          ...
          return {
            ...
            playerCount: playerCount.data || 0,
            availableSpots: availableSpots.data || 0,
            isUserMember: false // Featured view doesn't need user context
          };
        })
      );

      // Cache for 10 minutes
      this.setCache(cacheKey, featuredLeagues, 600);

      return { data: featuredLeagues, error: null, success: true };
    } catch (error) {
      return {
        data: null,
        error: this.handleError(error, 'getFeaturedLeagues'),
        success: false
      };
    }
  }
  ...
  /**
   * Get public leagues with enhanced filtering for browsing
   */
  async getPublicLeagues(
    filters: PublicLeagueFilters = {},
    options: {
      limit?: number;
      offset?: number;
    } = {}
    ...
    try {
      // Reuse existing discoverLeagues method with public filter
      const publicFilters = {
        ...filters,
        // Force only public leagues
      };

      // Override internal query to ensure only public leagues
      const result = await this.discoverLeagues(publicFilters, options);
      
      // Additional filtering for featured and open registration
      if (result.success && result.data) {
        let filteredLeagues = result.data;

        if (filters.featured) {
          filteredLeagues = filteredLeagues.filter(league => league.featured);
        }

        if (filters.openRegistration) {
          const now = new Date().toISOString();
          filteredLeagues = filteredLeagues.filter(league => 
            !league.registration_deadline || league.registration_deadline >= now
          );
        }
        ...
        return {
          ...
          pagination: {
            ...
            totalPages: Math.ceil(filteredLeagues.length / (options.limit || 20))
          }
        };
      }
      ...
      return {
        data: null,
        error: this.handleError(error, 'getPublicLeagues'),
        success: false,
        pagination: {
          page: 1,
          limit: options.limit || 20,
  ...
  /**
   * Check if league accepts new team registrations
   */
  async isLeagueOpenForRegistration(leagueId: string): Promise<ServiceResponse<{
    isOpen: boolean;
    reason?: string;
    details: {
      isPublic: boolean;
    ...
    try {
      ...
      const hasDeadlinePassed = league.registration_deadline ? league.registration_deadline < now : false;
      const isAtMaxTeams = league.max_teams ? currentTeams >= league.max_teams : false;

      const isOpen = league.is_public && !hasDeadlinePassed && !isAtMaxTeams;
      
      let reason: string | undefined;
      if (!league.is_public) reason = 'League is not public';
      else if (hasDeadlinePassed) reason = 'Registration deadline has passed';
      else if (isAtMaxTeams) reason = 'League has reached maximum teams';
      ...
      return {
        data: null,
        error: this.handleError(error, 'isLeagueOpenForRegistration'),
        success: false
      };
    }
  }
  ...
  /**
   * Get league publishing statistics for admin
   */
  async getLeaguePublishingStats(adminId: string): Promise<ServiceResponse<{
    totalLeagues: number;
    publishedLeagues: number;
    featuredLeagues: number;
    ...
    try {
      ...
      // Get team join requests for admin's leagues
      const leagueIds = leagues?.map(l => l.id) || [];
      const { count: requestsCount } = leagueIds.length > 0 ? await this.supabase
        .from('team_league_requests')
        .select('*', { count: 'exact', head: true })
        .in('league_id', leagueIds) : { count: 0 };

      const now = new Date().toISOString();
      const publishedLeagues = leagues?.filter(l => l.is_public) || [];
      const featuredLeagues = leagues?.filter(l => l.featured) || [];
      const activeRegistrations = leagues?.filter(l => 
        l.is_public && (!l.registration_deadline || l.registration_deadline >= now)
      ) || [];

      return {
        data: {
          totalLeagues: leagues?.length || 0,
          publishedLeagues: publishedLeagues.length,
          featuredLeagues: featuredLeagues.length,
          totalTeamRequests: requestsCount || 0,
      ...
      return {
        data: null,
        error: this.handleError(error, 'getLeaguePublishingStats'),
        success: false
      };
    }
  }
