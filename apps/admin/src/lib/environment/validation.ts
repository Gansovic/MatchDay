/**
 * Environment Validation Utility for MatchDay Admin
 * 
...
export function validateEnvironment(): ValidationResult {
  ...
  const playerApiUrl = process.env.NEXT_PUBLIC_PLAYER_API_URL
...
export function logEnvironmentStatus(): void {
  ...
  console.group('🔧 MatchDay Admin Environment Configuration')
  ...
  if (result.config) {
    ...
    if (config.playerApiUrl) {
      console.log(`Player API: ${config.playerApiUrl}`)
    }
...
export function validateEnvironmentAtStartup(): void {
  try {
    ...
    if (config.isRemote && config.isDevelopment) {
      console.warn('\n' + '='.repeat(60))
      console.warn('⚠️  WARNING: ADMIN APP USING PRODUCTION DATABASE!')
