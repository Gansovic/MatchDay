...
export function TeamLogoUpload({
  ...
  const handleCropComplete = async (croppedImage: Blob) => {
    console.log('🛡️ TeamLogoUpload - Starting upload...');
    setIsUploading(true);
    setError(null);
    ...
    try {
      ...
      console.log('🛡️ TeamLogoUpload - File created:', {
        name: file.name,
        size: file.size,
        type: file.type,
        teamId,
      ...
      console.log('🛡️ TeamLogoUpload - FormData prepared with context_type=team_logo, team_id=' + teamId);

      // Upload to API
      console.log('🛡️ TeamLogoUpload - Sending upload request to /api/media/upload...');
      const response = await fetch('/api/media/upload', {
        method: 'POST',
        body: formData,
      });

      console.log('🛡️ TeamLogoUpload - Upload response status:', response.status);

      if (!response.ok) {
        const errorData = await response.json();
        console.error('🛡️ TeamLogoUpload - Upload failed:', errorData);
      ...
      setImageToEdit(null);
      console.log('🛡️ TeamLogoUpload - Local state updated, calling onUploadComplete callback');
      ...
      console.error('🛡️ TeamLogoUpload - Upload error:', err);
      setError(err instanceof Error ? err.message : 'Failed to upload logo');
    } finally {
      setIsUploading(false);
      console.log('🛡️ TeamLogoUpload - Upload process completed');
    }
  };
