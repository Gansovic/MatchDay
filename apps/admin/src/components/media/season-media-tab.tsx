...
interface SeasonMediaTabProps {
  seasonId: string;
  seasonName: string;
  canUpload?: boolean;
}

export function SeasonMediaTab({
  seasonId,
  seasonName,
  canUpload = false
}: SeasonMediaTabProps) {
  ...
  const fetchMedia = async () => {
    try {
      setIsLoading(true);
      const response = await fetch(`/api/media?season_id=${seasonId}`);
      ...
      console.log('🎬 Admin SeasonMediaTab - Fetched media:', data);
      setMedia(data);
    } catch (err) {
  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      ...
      formData.append('context_type', 'season_media');
      formData.append('season_id', seasonId);
      formData.append('uploaded_by', user.id);
      formData.append('is_public', 'true'); // Season media is public - everyone can view it
      formData.append('description', `Media from ${seasonName}`);
      formData.append('tags', JSON.stringify(['season', seasonName.toLowerCase().replace(/\s+/g, '-')]));
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
            Photos and videos from {seasonName}
          </p>
        </div>
