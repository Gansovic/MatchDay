...
/**
 * Create 2023 Season Users
 * Generates 50+ realistic users for the 2023 football season
 */
...
const getRandomPosition = () => {
  ...
  if (rand < 0.08) return 'goalkeeper';     // ~8% goalkeepers (2-3 per team)
...
async function createUsers() {
  console.log('🚀 Creating 50+ realistic users for 2023 season...\n');
  ...
  for (let i = 0; i < 55; i++) {
    ...
    const preferredFoot = feet[Math.floor(Math.random() * feet.length)];
    ...
    const authUser = {
      ...
      role: 'authenticated', 
      email: email,
      encrypted_password: '$2a$10$hash', // Mock hash
  ...
  try {
    ...
    // Insert into auth.users table using raw SQL
    for (const user of users) {
      ...
      if (error && !error.message?.includes('already exists')) {
        console.error(`❌ Error inserting auth user ${user.email}:`, error);
      }
    }

    console.log('✅ Auth users inserted successfully!');
    ...
    userProfiles.slice(0, 5).forEach((user, index) => {
      console.log(`   ${index + 1}. ${user.full_name} - ${user.position} (${user.email})`);
    });
    
    console.log(`\n📧 All users can be accessed with emails like: ${userProfiles[0].email}`);
    console.log('🔐 All users have the same mock password for testing purposes.\n');
