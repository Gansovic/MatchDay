...
const envSchema = z.object({
    ...
    NEXT_PUBLIC_SUPABASE_URL: z
        ...
        .refine((url) => url.startsWith('http://127.0.0.1') || url.startsWith('https://'), 'Supabase URL must use http:// (local) or https:// (production)'),
    NEXT_PUBLIC_SUPABASE_ANON_KEY: z
        .string()
        .min(20, 'Supabase anon key seems too short - check your configuration')
        .refine((key) => key.startsWith('eyJ') || key.startsWith('sb_publishable_'), 'Supabase anon key must be a valid JWT or publishable key'),
    SUPABASE_SERVICE_ROLE_KEY: z
        .string()
        .min(20, 'Supabase service role key seems too short')
        .refine((key) => key.startsWith('eyJ') || key.startsWith('sb_secret_'), 'Supabase service role key must be a valid JWT or secret key')
 ...
 */
export function isProductionSupabase() {
 ...
 */
export function warnIfProductionInDev() {
    if (process.env.NODE_ENV === 'development' && isProductionSupabase()) {
        console.warn('⚠️  WARNING: You are running in DEVELOPMENT mode but connected to PRODUCTION Supabase!\n' +
