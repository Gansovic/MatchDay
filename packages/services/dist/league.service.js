...
export class LeagueService {
     ...
     */
    async createLeague(name, userId) {
        try {
            const leagueData = {
                name,
                sport_type: 'football',
     ...
     */
    async getAdminLeagues(adminId, options = {}) {
        try {
    ...
    async calculateCompatibilityScore(leagueId, userId) {
        try {
            ...
            const score = Math.round(factors.skillMatch * weights.skillMatch +
    ...
    calculateSkillMatch(league, userStats) {
        if (!userStats)
            return 50; // Neutral score for new players
        ...
        if (league.league_type === 'casual' && totalGames < 10)
            return 85;
        if (league.league_type === 'competitive' && avgGoalsPerGame > 0.5)
            return 80;
        if (league.league_type === 'friendly')
            return 75;
        return 60;
    }
    calculateLocationProximity(league, userProfile) {
        // Simplified location matching - would need geolocation in real implementation
        if (!league.location || !userProfile.location)
            return 50;
    ...
    calculateScheduleCompatibility(league) {
        ...
        if (seasonStart > now)
            return 85; // Upcoming season
        if (seasonEnd > now)
            return 70; // Current season
        return 30; // Past season
    }
    calculateTeamAvailability(league) {
        if (league.availableSpots === 0)
            return 0;
        if (league.availableSpots > 10)
            return 95;
        if (league.availableSpots > 5)
            return 80;
        return 60;
    }
     ...
     */
    subscribeToLeagueUpdates(leagueId, callback, options = { table: 'leagues', event: '*' }) {
