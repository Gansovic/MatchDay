...
export class StatsService {
    ...
    async calculatePerformanceTrends(userId, seasonYear, period = 'monthly', statTypes = ['goals', 'assists', 'performance']) {
        try {
    ...
    calculateOverallRating(crossLeagueStats, leagueStats) {
        if (!crossLeagueStats || !leagueStats.length)
            return 50;
        ...
        const rating = Math.min(100, Math.max(0, (avgGoalsPerGame * 20) +
    ...
    getCompetitivenessDescription(score) {
        if (score >= 80)
            return 'Highly competitive with balanced teams';
        if (score >= 60)
            return 'Good competitive balance';
        if (score >= 40)
     ...
     */
    subscribeToStatsUpdates(userId, callback, options = { table: 'player_stats', event: '*' }) {
     ...
     */
    async getPlayerSeasonStats(playerId, seasonYear) {
        try {
     ...
     */
    async getSeasonPlayerStats(seasonYear, options = {}) {
        try {
     ...
     */
    async validateStatsConsistency(seasonId) {
        try {
