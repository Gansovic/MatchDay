...
import { OnboardingData, SPORTS, EXPERIENCE_LEVELS } from '../types';
...
export const SportsStep: React.FC<SportsStepProps> = ({
  ...
  const selectedSports = formData?.preferred_sports || [];
  ...
  const handleSportToggle = (sportId: string) => {
    const newSelection = selectedSports.includes(sportId)
      ? selectedSports.filter(s => s !== sportId)
      : [...selectedSports, sportId];
    
    setValue('sports.preferred_sports', newSelection);
  };
  ...
  const getPopularPositions = () => {
    const positions = new Set<string>();
    selectedSports.forEach(sportId => {
      const sport = SPORTS.find(s => s.id === sportId);
      if (sport?.popularPositions) {
        sport.popularPositions.forEach(pos => positions.add(pos));
      }
    });
    return Array.from(positions).slice(0, 8); // Limit to 8 most common
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      {selectedSports.length > 0 && getPopularPositions().length > 0 && (
        <div>
          <h3 className="text-xl font-semibold text-gray-900 mb-4 flex items-center">
            <span className="mr-2">🎯</span>
            Preferred Position
          ...
          <div className="space-y-4">
            <input
              ...
              type="text"
              placeholder="e.g., Forward, Point Guard, Midfielder..."
            ...
            <div className="flex flex-wrap gap-2">
              <span className="text-sm text-gray-500">Popular positions:</span>
              {getPopularPositions().map((position) => (
                <button
                  key={position}
      ...
      {selectedSports.length > 0 && (
        <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-2xl p-6">
          <h3 className="font-bold text-gray-900 mb-3 flex items-center">
            ...
            Your Athlete Profile Preview
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            <div className="bg-white/70 rounded-xl p-4">
              <div className="font-semibold text-gray-700 mb-1">Sports</div>
              <div className="flex flex-wrap gap-1">
                {selectedSports.map(sportId => {
                  const sport = SPORTS.find(s => s.id === sportId);
                  return (
                    <span key={sportId} className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded-full">
