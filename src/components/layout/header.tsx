...
export const Header: React.FC<HeaderProps> = ({ className = '' }) => {
  ...
  useEffect(() => {
    async function getUserRole() {
      if (user) {
        ...
        setUserRole(role);
        setIsAdmin(role === 'league_admin' || role === 'app_admin');
      } else {
        setUserRole(null);
        setIsAdmin(false);
      }
  ...
  // Player app navigation - no admin features
  const navItems = [
              ...
              <div className="flex items-center space-x-3">
                ...
                {isAdmin && (
                  <a
                    ...
                    className="flex items-center space-x-2 px-3 py-2 bg-orange-600 hover:bg-orange-700 text-white font-medium rounded-lg transition-colors"
