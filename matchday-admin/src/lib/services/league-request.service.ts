...
import { ServiceResponse, ServiceError } from '@/lib/types/database.types';
  ...
  async approveRequest({ requestId, adminId, responseMessage }: ApproveRequestData): Promise<ServiceResponse<TeamLeagueRequest>> {
    try {
      ...
      try {
        ...
        const playerApiUrl = process.env.NEXT_PUBLIC_PLAYER_API_URL || 'http://localhost:3001';
        const response = await fetch(`${playerApiUrl}/api/league-requests/${requestId}/approve`, {
          method: 'POST',
          headers: {
  ...
  async rejectRequest({ requestId, adminId, responseMessage }: RejectRequestData): Promise<ServiceResponse<TeamLeagueRequest>> {
    try {
      ...
      try {
        ...
        const playerApiUrl = process.env.NEXT_PUBLIC_PLAYER_API_URL || 'http://localhost:3001';
        const response = await fetch(`${playerApiUrl}/api/league-requests/${requestId}/reject`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${session.access_token}`
